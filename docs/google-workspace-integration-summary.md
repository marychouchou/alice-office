...
| 路徑全靠 `~`（`~/.config/google-calendar-mcp` 等） | 共用目錄 `data/_google/` 掛載到容器 `/opt/google-workspace`，所有路徑用 manifest env 明確指定 | 容器內 gateway 與 MCP 子進程以 uid 10000（`hermes`）執行，`/root`（700）不可達；且 gateway 設 `XDG_CONFIG_HOME=/opt/data/.config`，「預設路徑」會被悄悄改到 per-room 目錄 |
...
- Settings：`GOOGLE_OAUTH_PUBLIC_URL`、`GOOGLE_OAUTH_GATE`；憑證放 `data/_google/`（gitignored）
...
### 5. 每次呼叫背後怎麼拿 access token（gmail/drive 共用同一份 `token_manager.py`）
...
1. 用 `GOOGLE_ACCOUNT_MODE`（= account_key）從共用 `tokens.json` 撈出這個房間的 token。
...
3. 換到新 token 後**直接改寫 `/opt/google-workspace/tokens.json`**（`save_all_tokens`）——
   跟 router 的 `google_oauth.py` 是同一份檔案、同一個 key，所以 MCP 換的新 token 下次
   router 判斷 gate 狀態時也看得到，不需要額外同步。
...
所以三個 MCP 及 router 讀寫的是同一份檔案，靠檔案系統本身序列化，沒有額外的鎖或協調機制。

### 6. 每個房間各自一份 MCP 子行程

每個 `hermes_<room_id>` container 都會各自 spawn 一份 `gmail`/`drive`/`google-calendar`
子行程（stdio，房間之間不共用行程），但都指向同一個掛載進來的 `/opt/google-workspace`，
所以「行程隔離、資料共用」——這也是為什麼一定要靠 `GOOGLE_ACCOUNT_MODE` 這個 env
在讀寫共用檔案時分辨「這是哪個房間/帳號的 token」。
...
3. Linux 部署時 `data/_google/` 需可被 uid 10000 讀寫（macOS Docker Desktop 不用管）
