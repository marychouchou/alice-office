...
- **不會一開始就要你授權**：加好友或進群組後可以馬上正常聊天、問一般問題，完全
  不需要先授權。只有當你真的請助理做**跟 Google 有關**的事（查行事曆、讀信、找
  雲端硬碟檔案）、而它發現你還沒接上 Google 帳號時，才會在那則回覆裡附上一個
  **只屬於你自己**的授權連結。
- **授權流程**：點擊連結 → 用你的 Google 帳號登入並同意 → 跳回 LINE 即完成。**不用
  再把問題傳一次**——你剛才問的那個問題會自動重新處理，答案會直接出現在 LINE
  對話裡。
- **可以做什麼**：
  - 行事曆：查詢／新增行程
  - 信箱：讀信、搜尋、寄信、回信
...
- **什麼時候會出現（重新）授權連結**：
  - 完全沒授權過，或授權已經失效——助理照常回答其他問題，只有這次跟 Google 有關
    的請求會換成一則授權連結；
  - 已經授權但少了雲端硬碟的存取權限——助理仍會正常回答行事曆／信箱相關問題，但
    會額外提醒你重新授權一次以啟用雲端硬碟功能。
- **群組聊天裡**：每個人各自用自己的 Google 帳號授權，不會因為你點過連結，其他人
  問行事曆／信箱就變成用你的帳號查。助理會在授權連結前面點名是誰的連結（例如
  「小明 請點此連結」），授權完成後也會在群組裡說一聲「小明 已完成 Google
  授權」，避免點錯人的連結。要注意：這個連結沒有再次驗證身分，理論上群組裡別人
  也點得開，所以連結上如果點名的不是你，不要幫忙點；請助理操作別人的行事曆／信箱
  一律是用**你自己**的帳號去查，看不看得到要看對方有沒有把資料分享給你。
- **如果你在群組裡完全沒有加這個官方帳號為好友**，LINE 不會提供你的身分給助理，
  助理會請你先加好友再問一次，這種情況不會出現授權連結。
- 你的授權資料只屬於你所在的這個聊天室（群組裡則是屬於你個人），不會被其他聊天室
  或其他人看到或使用。
...
**助理做出來的檔案，會用下載連結給你。** 請它把資料整理成 Word／Excel／Markdown
檔時，回覆裡會附一個網址，點開就能下載（檔名會自動帶上）。連結**約 24 小時後失效**，
過期後再跟助理說一次「剛才那份檔案再給我一次」，它會產生一個新的連結。

## 已知限制

- 助理目前只會用**文字**回覆你，不會主動傳圖片／語音／影片訊息回來——就算你請它
  畫圖或錄音，它也只能用文字描述結果。
- **LINE 不允許 bot 直接傳檔案給你**，所以助理產出的檔案一律是「下載連結」而不是
  聊天室裡的檔案訊息。連結是**誰拿到誰能下載**（跟你把檔案轉傳給別人一樣），所以
  不要把連結轉給不該看到那份檔案的人；超過期限（約 24 小時）連結就會失效。
...
每次都等。多數情況下這段準備在你加好友、或把助理拉進群組的當下就已經在背景默默
完成了，所以實際傳第一則訊息時通常感覺不到延遲；只有在準備還沒完成前你就搶先傳訊
息，才會遇到這 30–60 秒。等待期間，一對一聊天會看到 LINE 的「…」載入動畫，代表
助理正在處理（群組沒有這個動畫）。
...
最常見的原因是當初授權時少給了雲端硬碟的存取權限——這種情況助理會在你問行事曆／
信箱相關問題時，主動多推播一則重新授權提示（行事曆／信箱不受影響，照常可用）。
如果是授權在 Google 那邊過期或被你自己收回，則不會主動通知你，而是等你下次真的
請助理操作 Google 服務時，那則回覆才會換成一個新的授權連結。兩種情況都照著訊息
裡的連結重新走一次授權流程即可，不影響其他已經在用的功能。
