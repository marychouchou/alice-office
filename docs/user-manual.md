...
整理（2026-08-28）。如果你的部署方另外調整過設定（例如呼叫詞是哪幾個字），請以他們
另外告知的為準。
...
- **授權流程**：當你第一次要助理操作行事曆／信箱／雲端硬碟相關功能時，如果還沒
  授權，助理會回一則訊息附上授權連結；點擊連結 → 用你的 Google 帳號登入並同意
  → 跳回 LINE 即完成。完成後把原本的問題再問一次就可以了。
- **可以做什麼**：
  - 行事曆：查詢／新增行程
  - 信箱：讀信、搜尋、寄信、回信
...
**Q4：我很久沒傳訊息了，現在傳訊息助理好像「忘記」之前聊的內容？**
助理如果太久沒有互動（預設是超過一天沒有新訊息），下一次你傳訊息時會自動開始一段
新的對話，讓對話品質不會因為累積太久而變差。助理會拿到一份簡短的交接摘要，通常還
接得上大概的脈絡，但細節不一定記得，建議把重點再說一次。
