...
**Note (this project):** this repo's `SKILL.md` overrides several points above — it uses
flowchart syntax instead of native `C4Context`/`C4Container` Mermaid syntax (rendering
reasons), and files land in `docs/architecture-c4.md`, not `docs/architecture/`. This
README and the `references/` files are the unmodified upstream background material;
follow `SKILL.md` for what actually applies here.
