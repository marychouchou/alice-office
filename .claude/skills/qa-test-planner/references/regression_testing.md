...
**Note (this project):** the automated regression suite here is `uv run pytest`
(see `AGENTS.md`'s "Testing" section); this guide's manual suite/timing
structure is for supplementary manual passes only (e.g. via
`docs/testing-paths.md`), not a replacement for it. E-commerce examples above
don't map to this repo — see `SKILL.md` for what to actually check per change
(e.g. after touching `container_manager.py`).
