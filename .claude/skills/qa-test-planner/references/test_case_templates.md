...
**Note (this project):** the UI/Visual and Security(OWASP)/Performance(req/s)
templates above assume a web frontend and multi-instance deployment this repo
doesn't have. For actual usage here, prefer the simplified templates in
`SKILL.md` (webhook 解析／gate 判斷／container 生命週期／session 行為 types),
and don't invent performance numbers without a measured baseline.
