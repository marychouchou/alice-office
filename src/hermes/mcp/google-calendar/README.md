...
| `GOOGLE_CALENDAR_MCP_TOKEN_PATH` | tokens.json 路徑，**必須**明確設為 `/opt/google-workspace/tokens.json`（共用掛載），不能沿用套件預設路徑，理由見下方。 |
...
2. gateway 會設 `XDG_CONFIG_HOME=/opt/data/.config`，所以套件的「預設」
   ...
   而不是共用位置——token 會存錯地方且房間之間對不起來。

因此共用掛載點選在中立的 `/opt/google-workspace`（router 的
`container_manager.CONTAINER_GOOGLE_DIR`，掛的是 host 的 `data/_google/`），
所有 consumer（本 MCP、`gmail/`、`drive/`）一律吃 manifest `env:` 明確給的
絕對路徑。
...
只有當這個部署設定了 `GOOGLE_OAUTH_PUBLIC_URL` 且
