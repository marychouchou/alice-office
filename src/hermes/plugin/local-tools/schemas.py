...
OCR_SCHEMA = {
    "description": (
        "讀取 PDF 或圖片的文字與內容，支援繁體中文、印刷體、考卷、合約、一般文件與照片。"
        "PDF 支援多頁（預設最多 30 頁）：有文字層的頁直接抽出文字（快、不呼叫模型），"
        "只有掃描／拍照的頁才走視覺辨識；回傳的文字以「--- 第 N 頁 ---」分頁。"
        "圖片則做單張視覺辨識。結果有 SHA-256 cache，相同檔案不重複辨識。"
        "使用者從聊天室傳來的檔案存在 /opt/data/incoming/ 底下，直接把那個路徑傳進來即可。"
        "回傳 text（全文）、pages（頁數）、ocr_pages（真的走視覺辨識的頁碼），由呼叫端決定如何整理。"
        "重要：檔案內容不會留在你的記憶裡跨輪保存，先前讀過的內容在後續回合可能已經不在 context 中；"
        "因此在回答任何與某個檔案內容有關的問題之前，一律先重新呼叫這個工具取得內容，"
        "不要憑印象作答（有快取，重複呼叫幾乎是瞬間完成）。"
    ),
    "parameters": {
        "type": "object",
        "properties": {
            "path": {
                "type": "string",
                "description": (
                    "檔案絕對路徑（支援 .pdf/.jpg/.jpeg/.png/.webp）。"
                    "使用者傳來的檔案通常在 /opt/data/incoming/<檔名>。"
                ),
            },
            "prompt": {
                "type": "string",
                "description": (
                    "視覺辨識提示（選填，只有掃描頁／圖片用得到）。例如：「只抄錄題目與選項」、"
                    "「描述圖片場景」、「找出金額與日期」。留空則使用預設全文 OCR 提示。"
                ),
            },
        },
        "required": ["path"],
